...
public class RuntimeCircleClipper : MonoBehaviour, IClip
{
    ...
    public float diameter = 1.2f, radius = 1.2f, touchMoveDistance = 0.1f;

    public int segmentCount = 10;

    private Vector2f currentTouchPoint, previousTouchPoint;
    ...
    void UpdateTouch()
    {
        if (TouchUtility.TouchCount > 0)
        {
            ...
            {
                Debug.DrawRay(mainCamera.transform.position, ray.direction * 50, Color.red);
            }
            touchPhase = touch.phase;
            if (touch.phase == TouchPhase.Began)
            {
                Vector2 XOYPlaneLocation = hit.point;
                currentTouchPoint = XOYPlaneLocation - terrain.GetPositionOffset();
